	...
	try {
		...
		try {
			...
			*/
			...
			console.log('Failed to get task count from ToDokchain');
		}
	} catch {
...
/**
* removeTask() remove the task from ToDokchain and then from 
...
async function removeTask(taskIndex) {
	...
	try {
		...
		console.log('Remove Task ' + taskIndex + ' from the ToDokchain');
...
/**
* changeTaskStatus() change the status of task in ToDokchain and 
* then in the HTML
...
async function changeTaskStatus(id, taskIndex) {
	...
	try {
		...
		console.log('Failed to change Status of task ' + textId + ' in ToDokchain');
	}
}
...
* add it to ToDokchain and update the count via updateTaskCount() 
* @author Gupta Shrinath <https://github.com/gupta-shrinath>
* @param {string} name 
*/
async function addTask(name) {
	...
	if (form.checkValidity()) {
		console.log('Get the number of task from ToDokchain');
		...
		// Get the number of task from ToDokchain // 
		contract.methods.getTaskCount().call({ from: web3.eth.defaultAccount }).then(numberOfTask => {
			...
			console.log('Failed to get the number of task in ToDokchain ' + err);
		});
		try {
			await contract.methods.addTask(name).send({ from: web3.eth.defaultAccount });
			console.log('Add task ' + name + ' to ToDokchain');
		} catch {
